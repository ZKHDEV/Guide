...
public class AccountController {
	...
	public void login(HttpServletRequest request, HttpServletResponse response,
			@Validated LoginQueryVo loginQueryVo, BindingResult bindingResult) throws IOException{
	...
	public void register(HttpServletRequest request, HttpServletResponse response, 
			@Validated RegisterQueryVo registerQueryVo, BindingResult bindingResult) throws IOException, MessagingException{
		...
		if(vCode.equalsIgnoreCase(registerQueryVo.getRegcode().trim())){
			if(bindingResult.hasErrors()){
				...
				response.getWriter().write("no:"+errormsg);
			}else if(!registerQueryVo.getRegpwd().equalsIgnoreCase(registerQueryVo.getRregpwd())){
				response.getWriter().write("no:两次密码输入不匹配。");
			}else if(userService.existedUser(registerQueryVo.getReguser().trim())){
				response.getWriter().write("no:用户名已存在。");
			}else if(userService.existedEmail(registerQueryVo.getRegemail().trim())){
				response.getWriter().write("no:邮箱已存在。");
			}else{
				userService.register(registerQueryVo);
				
				GuideUser user = userService.selectByUserName(registerQueryVo.getReguser());
				//发送激活码
				Properties prop = new Properties();
				prop.load(this.getClass().getClassLoader().getResourceAsStream("mail.properties"));
				String content = prop.getProperty("content");
				String msg = MessageFormat.format(content, user.getUserName(), user.getUserVerify());
				Session mailSession = MailUtils.createSession(prop.getProperty("host"), prop.getProperty("username"), prop.getProperty("password"));
				Mail mail = new Mail(prop.getProperty("from"), user.getUserEmail(), prop.getProperty("subject"), msg);
	...
	public void checkUser(HttpServletResponse response,String username) throws IOException{	
		if(!username.trim().isEmpty() && username!=null){
			response.getWriter().write(userService.existedUser(username.trim())?"false":"true");
		}	
	}
	...
	public void checkEmail(HttpServletResponse response,String email) throws IOException{
		if(!email.trim().isEmpty() && email!=null){
			response.getWriter().write(userService.existedEmail(email.trim())?"false":"true");
		}
	...
	public void forgetPwd(HttpServletRequest request,HttpServletResponse response,
			@Validated ForgetQueryVo forgetQueryVo, BindingResult bindingResult) throws IOException, MessagingException{
		response.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		HttpSession session = request.getSession();
		String vCode = session.getAttribute("vCode").toString();
		if(vCode.equalsIgnoreCase(forgetQueryVo.getFgcode().trim())){
			if(bindingResult.hasErrors()){
				...
				GuideUser user = userService.selectByUserName(forgetQueryVo.getFgname());
				...
				String msg = MessageFormat.format(content, user.getUserName(), user.getUserVerify());
				Session mailSession = MailUtils.createSession(prop.getProperty("host"), prop.getProperty("username"), prop.getProperty("password"));
				Mail mail = new Mail(prop.getProperty("from"), user.getUserEmail(), "密码重置", msg);
				MailUtils.send(mailSession, mail);
	...
	public void changePwd(HttpServletRequest request,HttpServletResponse response,
			@Validated ChangePwdQueryVo changePwdQueryVo, BindingResult bindingResult) throws IOException{
		response.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		HttpSession session = request.getSession();
		...
		if(vCode.equalsIgnoreCase(changePwdQueryVo.getCode().trim())){
			if(bindingResult.hasErrors()){
				...
				response.getWriter().write("no:"+errormsg);
			}else if(!changePwdQueryVo.getNewpwd().equalsIgnoreCase(changePwdQueryVo.getReppwd())){
				response.getWriter().write("no:两次密码输入不匹配。");
			}else if(!changePwdQueryVo.getOldpwd().equalsIgnoreCase(user.getUserPwd())){
				response.getWriter().write("no:原密码错误。");
			}else if(userService.changePwd(user, changePwdQueryVo.getNewpwd())){
	...
	public void resetPwd(HttpServletRequest request,HttpServletResponse response,
			@Validated ResetPwdQueryVo resetPwdQueryVo, BindingResult bindingResult) throws IOException{
		...
		if(bindingResult.hasErrors()){
			...
			response.getWriter().write("no:"+errormsg);
		}else if(!resetPwdQueryVo.getFgnewpwd().trim().equalsIgnoreCase(resetPwdQueryVo.getFgreppwd().trim())){
			response.getWriter().write("no:两次密码输入不匹配");
		}else{
			GuideUser user = userService.selectByPrimaryKey(resetPwdQueryVo.getFguserid());
			
			if(user.getUserVerify().equalsIgnoreCase(resetPwdQueryVo.getFgcode2().trim())){
	...
	@RequestMapping(value="/Account/changeUser",method={RequestMethod.POST})
	public void changeUser(HttpServletRequest request,HttpServletResponse response,
			@Validated ChangeUNameQueryVo changeUNameQueryVo, BindingResult bindingResult) throws IOException{
		...
		if(vCode.equalsIgnoreCase(changeUNameQueryVo.getUcode().trim())){
			if(bindingResult.hasErrors()){
				...
				response.getWriter().write("no:"+errormsg);
			}else if(userService.existedUser(changeUNameQueryVo.getUname().trim())){
				response.getWriter().write("no:用户名已存在。");
			}else if(!changeUNameQueryVo.getUpwd().equalsIgnoreCase(user.getUserPwd())){
				response.getWriter().write("no:密码错误");
			}else {
				user.setUserName(changeUNameQueryVo.getUname().trim());
				
				if(userService.changeUserName(user, changeUNameQueryVo.getUname())){
					user = userService.selectByPrimaryKey(user.getUserId());
					session.setAttribute("login_user", user);
	...
	public void changeEmail(HttpServletRequest request,HttpServletResponse response,
			@Validated ChangeEmailQueryVo changeEmailQueryVo, BindingResult bindingResult) throws IOException, MessagingException{
		...
		GuideUser user = (GuideUser)session.getAttribute("login_user");
		String vCode = session.getAttribute("vCode").toString();
		if(vCode.equalsIgnoreCase(changeEmailQueryVo.getEcode().trim())){
			if(bindingResult.hasErrors()){
				List<ObjectError> allErrors = bindingResult.getAllErrors();
				String errormsg = "";
				for(ObjectError objectError:allErrors){
				...
				response.getWriter().write("no:"+errormsg);
			}else if(userService.existedUser(changeEmailQueryVo.getEmail().trim())){
				response.getWriter().write("no:用户名已存在。");
			}else if(!changeEmailQueryVo.getEpwd2().equalsIgnoreCase(user.getUserPwd())){
				response.getWriter().write("no:密码错误");
			}else {			
				if(userService.changeEmail(user, changeEmailQueryVo.getEmail().trim())){
					user = userService.selectByPrimaryKey(user.getUserId());
					session.setAttribute("login_user", user);
	...
	@RequestMapping(value="/Account/verifyEmail",method={RequestMethod.POST})
	public void verifyEmail(HttpServletRequest request,HttpServletResponse response,
		...
		HttpSession session = request.getSession();
		GuideUser user = (GuideUser)session.getAttribute("login_user");
		if(user.getUserVerify().equalsIgnoreCase(vcode.trim())){		
			if(userService.updateStateByPrimaryKey(user,(byte)1)){
				response.getWriter().write("ok:邮箱验证成功。");
			}else{
				response.getWriter().write("no:邮箱验证失败，请重试。");
			}		
		}else{
			response.getWriter().write("no:验证码错误。");
		}
	...
	@RequestMapping("/Account/sendVerify")
	public void sendVerify(HttpServletRequest request,HttpServletResponse response,
		...
		try {
			MailUtils.send(mailSession, mail);
			response.getWriter().write("ok:验证码发送成功。");
		} catch (MessagingException e) {
			response.getWriter().write("no:验证码发送失败，请重试。");
		}	
	}
